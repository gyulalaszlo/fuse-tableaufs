  ...
  struct Dispatcher {
    ...
    public:
      ...
      void register_op( int op_id, HandlerT handler ) {
        ops.emplace( std::make_pair( op_id, std::make_unique<OpAdapter<InputT, OutputT, HandlerT>>(handler)));
      }
    ...
    private:
      ...
      class Op {
        MKZ_INTERFACE(Op);
        virtual void run( StateT& state, InputStream& input, OutputStream& output  ) = 0;
      };
      ...
      class OpAdapter : public Op {
        ...
        virtual void run( StateT& state, InputStream& in_stream, OutputStream& output  ) {
