  ...
  namespace workers
  {
    class Readdir
    {
     ...
     private:
      ...
      static void add_directory_entries(Output& output, const PGQuery& query)
      {
        ...
        for (auto i = 0; i < size; ++i) {
          dirs.set(i + 2,
                   sanitize_filename(query.get_string(i, TFS_WG_QUERY_NAME)));
        }
     ...
     public:
      ...
      {
        const auto path_res =
            parse_tableau_path(((std::string)input.getPath()).c_str());
        ...
        switch (path.level) {
          ...
          case PathNode::Site:
            add_directory_entries(
                output,
                PGQuery(state.connection.get(), TFS_WG_LIST_PROJECTS,
          ...
          case PathNode::Project:
            add_directory_entries(
                output,
                PGQuery(state.connection.get(), TFS_WG_LIST_WORKBOOKS
