...
namespace tableauFS
{
  ...
  {
    encode_path<proto::ReaddirReq>(out, path);
  }
  CAPNP_DECODE_FN(read_dir_req, std::string& path)
  {
    decode_path<proto::ReaddirReq>(in, path);
  }
  ...
  {
    encode_path<proto::GetAttributesReq>(out, path);
  }
  CAPNP_DECODE_FN(get_attributes_req, std::string& path)
  {
    decode_path<proto::GetAttributesReq>(in, path);
  }
